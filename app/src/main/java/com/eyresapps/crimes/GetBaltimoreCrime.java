...
public class GetBaltimoreCrime extends AsyncTask<String, String, ArrayList<ArrayList<Crimes>>> {
    ...
    public GetBaltimoreCrime(Context context, boolean filterByCrime, boolean filterByMonth, boolean firstLoad, int attempts) {
    ...
    protected ArrayList<ArrayList<Crimes>> doInBackground(String... params) {
        ...
        try {
            ...
            Log.i("BALTIMORE ","https://data.baltimorecity.gov/resource/icjs-e3jg.json?$where=within_circle(location_1, " + latLng.getLatLng().latitude + ", " + latLng.getLatLng().longitude + ", 1000) and arrestdate between '" + dateUtil.getYear() + "-" + dateUtil.getMonth() + "-01T00:00:00,000' and '" + dateUtil.getYearAhead() + "-" + dateUtil.getMonthAhead() + "-01T00:00:00.000'");
    ...
    private void getCrimes(JSONArray jsonArray, int start, int end) {
        try {
            ...
            crime = (new Crimes(jsonArray.getJSONObject(i).getString("chargedescription"),
                    jsonArray.getJSONObject(i).getString("arrestdate"),
                    "",
                    "",
                    jsonArray.getJSONObject(i).getString("arrestlocation"),
                    ...
                    "",
                    jsonArray.getJSONObject(i).getString("chargedescription"),
                    jsonArray.getJSONObject(i).getString("arresttime"),
                    "",
            ...
            if (crimeList.isEmpty()) {
                ...
                for (int j = 0; j < crimeList.size(); j++) {
                    if (crimeList.get(j).get(0).getLatitude() ==
                            jsonArray.getJSONObject(position).getJSONObject("location_1").getJSONArray("coordinates").getDouble(1) &&
                            crimeList.get(j).get(0).getLongitude() ==
                                    jsonArray.getJSONObject(position).getJSONObject("location_1").getJSONArray("coordinates").getDouble(0)) {
                ...
                if (firstOfItsKind && jsonArray.getJSONObject(position).has("arrestlocation")) {
                    crimes = new ArrayList<>();
                    crimes.add(crime);
...
//                        }
    ...
    protected void onPostExecute(ArrayList<ArrayList<Crimes>> list) {
        ...
        if (list != null && !list.isEmpty()) {
            ...
            attempts++;
            new GetBaltimoreCrime(context, filterByCrime,filterByMonth, firstLoad, attempts).execute("https://data.baltimorecity.gov/resource/icjs-e3jg.json?$where=within_circle(location_1, " + latLng.getLatLng().latitude + ", " + latLng.getLatLng().longitude + ", 1000) and arrestdate between '" + dateUtil.getYear() + "-" + dateUtil.getMonth() + "-01T00:00:00,000' and '" + dateUtil.getYearAhead() + "-" + dateUtil.getMonthAhead() + "-01T00:00:00.000'");
        } else {
