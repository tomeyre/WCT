...
public class GetSeattleCrime extends AsyncTask<String, Integer, ArrayList<ArrayList<Crimes>>> {
    ...
    public GetSeattleCrime(Context context, boolean filterByCrime, boolean filterByMonth, boolean firstLoad, int attempts) {
    ...
    private void getCrimes(JSONArray jsonArray, int start, int end) {
        try {
            for (int i = start; i < end; i++) {
                ...
                crime = (new Crimes(jsonArray.getJSONObject(i).getString("event_clearance_group"),
                        jsonArray.getJSONObject(i).getString("at_scene_time"),
                        jsonArray.getJSONObject(i).getString("event_clearance_date"),
                        "",
                        jsonArray.getJSONObject(i).getString("hundred_block_location"),
                        jsonArray.getJSONObject(i).getDouble("latitude"),
                        jsonArray.getJSONObject(i).getDouble("longitude"),
                        "",
                        jsonArray.getJSONObject(i).getString("event_clearance_description"),
                        "",
                        "",
                        ""));
    ...
    private synchronized void addToList(JSONArray jsonArray, int position, final Crimes crime){
        try {
            if (crimeList.isEmpty()) {
                ...
                if (firstOfItsKind && jsonArray.getJSONObject(position).has("hundred_block_location")) {
                    crimes = new ArrayList<>();
                    crimes.add(crime);
