...
public class GetBatonRougeCrime extends AsyncTask<String, String, ArrayList<ArrayList<Crimes>>> {
    ...
    public GetBatonRougeCrime(Context context, boolean filterByCrime, boolean filterByMonth, boolean firstLoad, int attempts) {
    ...
    protected ArrayList<ArrayList<Crimes>> doInBackground(String... params) {
        ...
        try {
            ...
            Log.i("BATON ROUGE ","https://data.brla.gov/resource/5rji-ddnu.json?$where=within_circle(geolocation, " + latLng.getLatLng().latitude + ", " + latLng.getLatLng().longitude + ", 1000) and offense_date between '" + dateUtil.getYear() + "-" + dateUtil.getMonth() + "-01T00:00:00,000' and '" + dateUtil.getYearAhead() + "-" + dateUtil.getMonthAhead() + "-01T00:00:00.000'");
    ...
    private void getCrimes(JSONArray jsonArray, int start, int end) {
        try {
            ...
            crime = (new Crimes(jsonArray.getJSONObject(i).getString("crime"),
                    jsonArray.getJSONObject(i).getString("offense_date"),
                    "",
                    "",
                    ...
                    "",
                    jsonArray.getJSONObject(i).getString("offense_desc"),
                    jsonArray.getJSONObject(i).getString("offense_time"),"",jsonArray.getJSONObject(i).getString("file_number")));
...
//                        }
    ...
    protected void onPostExecute(ArrayList<ArrayList<Crimes>> list) {
        ...
        if (list != null && !list.isEmpty()) {
            ...
            attempts++;
            new GetBatonRougeCrime(context, filterByCrime,filterByMonth, firstLoad, attempts).execute("https://data.brla.gov/resource/5rji-ddnu.json?$where=within_circle(geolocation, " + latLng.getLatLng().latitude + ", " + latLng.getLatLng().longitude + ", 1000) and offense_date between '" + dateUtil.getYear() + "-" + dateUtil.getMonth() + "-01T00:00:00,000' and '" + dateUtil.getYearAhead() + "-" + dateUtil.getMonthAhead() + "-01T00:00:00.000'");
        } else {
