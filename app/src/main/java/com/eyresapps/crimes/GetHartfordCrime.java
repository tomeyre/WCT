...
public class GetHartfordCrime extends AsyncTask<String, String, ArrayList<ArrayList<Crimes>>> {
    ...
    public GetHartfordCrime(Context context, boolean filterByCrime, boolean filterByMonth, boolean firstLoad, int attempts) {
    ...
    protected ArrayList<ArrayList<Crimes>> doInBackground(String... params) {
        ...
        try {
            ...
            Log.i("HARTFORD ","https://data.hartford.gov/resource/h9if-m432.json?$where=within_circle(geom, " + latLng.getLatLng().latitude + ", " + latLng.getLatLng().longitude + ", 1000) and date between '" + dateUtil.getYear() + "-" + dateUtil.getMonth() + "-01T00:00:00' and '" + dateUtil.getYearAhead() + "-" + dateUtil.getMonthAhead() + "-01T00:00:00'");
    ...
    private void getCrimes(JSONArray jsonArray, int start, int end) {
        try {
            ...
            crime = (new Crimes(jsonArray.getJSONObject(i).getString("ucr_1_category"),
                    jsonArray.getJSONObject(i).getString("date"),
                    "",
                    "",
                    jsonArray.getJSONObject(i).getString("address"),
                    jsonArray.getJSONObject(i).getJSONObject("geom").getJSONArray("coordinates").getDouble(1),
                    jsonArray.getJSONObject(i).getJSONObject("geom").getJSONArray("coordinates").getDouble(0),
                    "",
                    jsonArray.getJSONObject(i).getString("ucr_1_description"),
                    jsonArray.getJSONObject(i).getString("time_24hr"),"",jsonArray.getJSONObject(i).getString("case_number")));
            ...
            if (crimeList.isEmpty()) {
                ...
                for (int j = 0; j < crimeList.size(); j++) {
                    if (crimeList.get(j).get(0).getLatitude() ==
                            jsonArray.getJSONObject(position).getJSONObject("geom").getJSONArray("coordinates").getDouble(1) &&
                            crimeList.get(j).get(0).getLongitude() ==
                                    jsonArray.getJSONObject(position).getJSONObject("geom").getJSONArray("coordinates").getDouble(0)) {
